...
final class VideoWindow {
 private static final int MIN=12,MAX=24;
 ...
 private long last,ack,peakUntil;private double rttMs=-1,peakRttMs;
 VideoWindow(){this(()->System.nanoTime()/1000000);}
 VideoWindow(LongSupplier clock){this.clock=clock;}
 synchronized int limit(){return rttMs<0?MAX:Math.max(MIN,Math.min(MAX,(int)Math.ceil((Math.max(rttMs,peakRttMs)+150)*30/1000)));}
 ...
 synchronized void acknowledge(long pts){
  ...
  long now=clock.getAsLong(),sample=Math.max(0,now-matched.at);
  // Retain recent path jitter for ten seconds instead of shrinking every ACK.
  if(sample>=peakRttMs||now>=peakUntil){peakRttMs=sample;peakUntil=now+10000;}
  // Grow promptly on delayed ACKs; shrink slowly after the path recovers.
  rttMs=rttMs<0?sample:sample>rttMs?sample:rttMs*.995+sample*.005;
