        ...
        try(BufferedReader input=new BufferedReader(new InputStreamReader(System.in,StandardCharsets.UTF_8))){
            String line;while((line=input.readLine())!=null && alive){
                ...
                try{
                    ...
                    else if(type.equals("video")){try{video.bitrate=Math.max(384000,Math.min(1200000,c.optInt("bitrate",1200000)));video.enable(c.optBoolean("enabled"));}catch(Exception e){event(json("videoStatus").put("available",false).put("message","تعذّر بدء الفيديو العتادي."));}}
